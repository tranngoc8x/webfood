...
{			
	var strchon="";
	var isChecked=true;
	var alen=document.getElementById('mainform').chk.length;				
	if (alen>1)
	{
		for(var i=0;i<alen;i++)
			if(document.getElementById('mainform').chk[i].checked==true)
				strchon+=document.getElementById('mainform').chk[i].value+",";
			else
				isChecked=false;
	}else
	{
		if(document.getElementById('mainform').chk.checked==true)
			strchon=document.getElementById('mainform').chk.value;
		else
			isChecked=false;
	}
	str=strchon;	
...
function check(re,tbl,flid){
	...
	if (conf){
		document.location.href='delete.php?re='+re+'&tbl='+tbl+'&fldid='+flid+'&val='+str;
		return false;
	}
}
function update(re,tbl,flid,flup,cont){
	calculatechon();
	if(str==''){
	alert("Bạn cần chọn ít nhất 1 mục để cập nhật !");	
	return;
	}
	var conf=confirm("Bạn có chắc chắn muốn cập nhật các mục đã chọn không ?");
	if (conf){
		window.location.reload();
		document.location.href='update.php?re='+re+'&tbl='+tbl+'&fldid='+flid+'&fldup='+flup+'&cont='+cont+'&val='+str;
